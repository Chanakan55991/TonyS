/*
TONY_GPS - a small GPS library for Arduino providing basic NMEA parsing
...
int TONY_GPS::available(void)
{
    return uartAvailable(_uart);
}
int TONY_GPS::availableForWrite(void)
{
    return uartAvailableForWrite(_uart);
}

int TONY_GPS::peek(void)
{
    if (available()) {
...
size_t TONY_GPS::write(uint8_t c)
{
	uartWrite(_uart, c);
    return 1;
}

size_t TONY_GPS::write(const uint8_t *buffer, size_t size)
{
	uartWriteBuf(_uart, buffer, size);
    return size;
}
uint32_t  TONY_GPS::baudRate()

{
	return uartGetBaudRate(_uart);
}
TONY_GPS::operator bool() const
{
    return true;
}
...
#ifndef _GPS_NO_STATS
void TONY_GPS::stats(unsigned long *chars, unsigned short *sentences, unsigned short *failed_cs)
{
...
unsigned long TONY_GPS::parse_decimal()
{
  char *p = _term;
  bool isneg = *p == '-';
  if (isneg) ++p;
...
unsigned long TONY_GPS::parse_degrees()
{
  char *p;
...
long TONY_GPS::gpsatol(const char *str)
{
  long ret = 0;
...
int TONY_GPS::gpsstrcmp(const char *str1, const char *str2)
{
...
/* static */
float TONY_GPS::distance_between (float lat1, float long1, float lat2, float long2) 
{
...
float TONY_GPS::course_to (float lat1, float long1, float lat2, float long2) 
{
...
const char *TONY_GPS::cardinal (float course)
{
...
void TONY_GPS::get_position(long *latitude, long *longitude, unsigned long *fix_age)
{
...
void TONY_GPS::get_datetime(unsigned long *date, unsigned long *time, unsigned long *age)
{
  if (date) *date = _date;
  if (time) *time = _time;
...
void TONY_GPS::f_get_position(float *latitude, float *longitude, unsigned long *fix_age)
{
  long lat, lon;
...
void TONY_GPS::crack_datetime(int *year, byte *month, byte *day, 
